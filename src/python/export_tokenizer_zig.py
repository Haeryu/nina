...
# === 저장용 JSON + TXT 파일 생성
tokenizer.model.save(f"./{data_dir}", prefix)  # my_bpe-vocab.json, my_bpe-merges.txt

# === vocab 불러오기
with open(f"{data_dir}/{prefix}-vocab.json", "r", encoding="utf-8") as vf:
    vocab = json.load(vf)
...
with open(f"{zig_output_dir}/merges_data.zig", "w", encoding="utf-8") as f:
    ...
    with open(f"{data_dir}/{prefix}-merges.txt", "r", encoding="utf-8") as mf:
        for line in mf:
            line = line.strip()
